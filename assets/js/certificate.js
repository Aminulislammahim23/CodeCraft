...
function generateShareLink() {
  ...
  const date = encodeURIComponent(document.getElementById("completionDate").value);

  if (!name || !date) {
    alert("Please enter your name and completion date first!");
    return;
  }
  ...
  navigator.clipboard.writeText(shareUrl).then(() => {
    alert("Shareable link copied! You can paste it anywhere (LinkedIn, Facebook, WhatsApp, etc.).");
  });
...
window.onload = function () {
  ...
  if (name) {
    document.getElementById("studentName").value = decodeURIComponent(name);
    document.getElementById("nameDisplay").textContent = decodeURIComponent(name);
  }

  if (date) {
    document.getElementById("completionDate").value = decodeURIComponent(date);
    document.getElementById("dateDisplay").textContent = decodeURIComponent(date);
  }
};
